...
DEFAULT_GNAIGPT_MODEL = os.getenv("GNAIGPT_MODEL", "claude-sonnet-4-5")
GNAIGPT_TOKEN = os.getenv("GNAIGPT_TOKEN", "")
GNAIGPT_API_URL = os.getenv("GNAIGPT_API_URL", "https://gnai.intel.com/api/providers/anthropic")
INTERNAL_GNAIGPT_HOSTS = {"gnai.intel.com"}


def get_gnaigpt_base_url() -> str:
    """Normalize configured URL into a base URL for the Anthropic client."""
    configured_url = GNAIGPT_API_URL.strip().rstrip("/")
...
def has_gnaigpt_config() -> bool:
    return bool(GNAIGPT_TOKEN and GNAIGPT_API_URL)


def is_internal_gnaigpt_url(url: str) -> bool:
    return (urlparse(url).hostname or "") in INTERNAL_GNAIGPT_HOSTS


def create_anthropic_client() -> Anthropic:
    """Create Anthropic client with optional custom base URL."""
    base_url = get_gnaigpt_base_url()
    use_internal_settings = is_internal_gnaigpt_url(base_url)
...
def call_gnaigpt_with_tools(
    ...
    try:
        ...
        print(
            "Anthropic API error "
            f"base_url={get_gnaigpt_base_url()} model={DEFAULT_GNAIGPT_MODEL}: {error}"
        )
        return "", None, None
    finally:
        client.close()
...
def build_orm_query(db: Session, params: ORMQueryParams) -> list[dict[str, Any]]:
    ...
    def resolve_report_expression(raw_name: str) -> tuple[str, Any] | None:
        """Resolve a raw select/group token into a SQLAlchemy expression.

        Supports plain report columns and a small set of date bucketing expressions
        generated by LLMs (for example strftime('%Y-%m', date)).
        """
        token = raw_name.strip()

        # Plain report column
        column = getattr(models.Report, token, None)
        if column is not None:
            return token, column

        # Compatibility for function-style date grouping tokens
        strftime_match = re.fullmatch(r"strftime\(\s*'([^']+)'\s*,\s*date\s*\)", token, flags=re.IGNORECASE)
        if strftime_match:
            fmt = strftime_match.group(1)
            if fmt == "%Y-%m":
                return token, func.to_char(models.Report.date, "YYYY-MM")
            if fmt == "%Y":
                return token, func.to_char(models.Report.date, "YYYY")
            if fmt == "%Y-%m-%d":
                return token, func.to_char(models.Report.date, "YYYY-MM-DD")

        # Support explicit date_trunc style grouping
        trunc_match = re.fullmatch(r"date_trunc\(\s*'([^']+)'\s*,\s*date\s*\)", token, flags=re.IGNORECASE)
        if trunc_match:
            grain = trunc_match.group(1).lower()
            if grain == "month":
                return token, func.to_char(func.date_trunc("month", models.Report.date), "YYYY-MM")
            if grain == "year":
                return token, func.to_char(func.date_trunc("year", models.Report.date), "YYYY")
            if grain == "day":
                return token, func.to_char(func.date_trunc("day", models.Report.date), "YYYY-MM-DD")
    ...
    if params.select_columns:
        for col_name in params.select_columns:
            resolved = resolve_report_expression(col_name)
            if resolved is None:
                continue
            alias, expression = resolved
        ...
        for index, aggregation in enumerate(params.aggregations, start=1):
            ...
            if aggregation.function == "count":
                if aggregation.column == "*":
                    # Force count(*) to anchor on report table instead of generating a FROM-less count(1).
                    expression = func.count(models.Report.id)
    ...
    if params.group_by:
        ...
        for col_name in params.group_by:
            ...
            if resolved is not None:
                _, expression = resolved
                group_by_columns.append(expression)
        if group_by_columns:
...
def handle_chat_ask(
    ...
    if not has_gnaigpt_config():
        return ChatAskResponse(
            answer="GNAIGPT service is not configured. Please contact administrator.",
            trace_id=trace_id,
        )
    ...
    response_text, tool_params, tool_use_id = call_gnaigpt_with_tools(
        messages=messages,
    ...
    final_answer, _, _ = call_gnaigpt_with_tools(messages=messages, temperature=0, max_tokens=2000)
